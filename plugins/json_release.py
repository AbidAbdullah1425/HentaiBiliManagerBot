...
# CHANGE: Run an async coroutine on the current event loop from a worker thread.
def _run_coro_in_loop(loop: asyncio.AbstractEventLoop, coro):
...
async def json_release(client: Client, message: Message):
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=download_workers) as executor:
            # CHANGE: Create task list and run downloads in parallel (max 3 at a time by default).
            for job in download_jobs:
                ...
                job["download_future"] = asyncio.wrap_future(executor.submit(_run_coro_in_loop, loop, coro))

            # CHANGE: Keep existing per-item flow order; just await downloads that are already running in parallel.
            for job in download_jobs:

                url = job["url"]
                studio = job["studio"]
                genres = job["genres"]
                genre_text = job["genre_text"]
                cover = job["cover"]
                preview_images_urls = job["preview_images_urls"]
                video_url = job["video_url"]
                title = job["title"]
                download_msg = job["download_msg"]

                try:
                    success, result = await job["download_future"]
